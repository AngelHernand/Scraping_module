...
namespace InterviewSimulator.Scraping.Scrapers.Epitech;

/// <summary>
/// Scraper para Epitech España (epitech-it.es) — escuela de tecnología española
/// con artículos de preguntas de entrevista para programadores en español.
/// </summary>
public class EpitechScraper : BaseScraper
{
    private static readonly List<(string Url, string Category, string Tech)> KnownPages = new()
    {
        ("https://www.epitech-it.es/preguntas-entrevista-programador/", "backend", "backend"),
        ("https://www.epitech-it.es/preguntas-entrevista-desarrollador-web/", "frontend", "frontend"),
        ("https://www.epitech-it.es/preguntas-entrevista-ingeniero-software/", "backend", "software-engineering"),
        ("https://www.epitech-it.es/preguntas-entrevista-data-science/", "data-science", "data-science"),
        ("https://www.epitech-it.es/preguntas-entrevista-devops/", "devops", "devops"),
    };

    public override string SourceName => "Epitech";
    public override SourceType SourceType => SourceType.BlogPlatform;

    public EpitechScraper(
        ILogger<EpitechScraper> logger,
    ...
    public override async Task<ScrapingResult> ScrapeAsync(CancellationToken cancellationToken = default)
    {
        ...
        Logger.LogInformation("[Epitech] Iniciando scraping — {Count} páginas conocidas", KnownPages.Count);

        // Primero intentar descubrir más URLs vía sitemap
        ...
        foreach (var (url, category, tech) in allUrls)
        {
            ...
            try
            {
                ...
                if (count > 0)
                    Logger.LogInformation("[Epitech] {Tech}: {Count} preguntas — {Url}", tech, count, url);
            }
            catch (HttpRequestException ex) when (
                ex.StatusCode == System.Net.HttpStatusCode.NotFound ||
                ex.StatusCode == System.Net.HttpStatusCode.Forbidden)
            {
                Logger.LogDebug("[Epitech] {Url} → {Code}", url, ex.StatusCode);
            }
            catch (Exception ex)
            {
                Logger.LogWarning(ex, "[Epitech] Error en {Url}", url);
        ...
        Logger.LogInformation("[Epitech] Completado — {Total} preguntas en {Elapsed:mm\\:ss}",
    ...
    private async Task<List<string>> DiscoverFromSitemapAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            var xml = await HttpClient.GetStringAsync("https://www.epitech-it.es/sitemap.xml", ct);
            var pattern = new System.Text.RegularExpressions.Regex(
                @"<loc>(https?://www\.epitech-it\.es/[^<]*(?:pregunta|entrevista)[^<]*)</loc>",
            ...
            if (urls.Count > 0)
                Logger.LogInformation("[Epitech] Descubiertas {Count} URLs de entrevista en sitemap", urls.Count);
        }
        catch (Exception ex)
        {
            Logger.LogDebug("[Epitech] Sitemap no disponible: {Msg}", ex.Message);
        }
        ...
        string url, string category, string tech, ScrapingResult result, CancellationToken ct)
    {
        ...
        foreach (var (questionText, answerText) in qaPairs)
        {
            ...
            q.Tags = $"[\"{tech}\",\"{category}\",\"interview\",\"epitech\"]";
